...
*Part 5 of **Fast Code, Slow Engineering**, a short series on what AI agents are actually speeding up — and what they aren't.*

> If humans are becoming the bottleneck, one of the more practical answers is to make the repository itself easier to reason about — for people and for agents. This is what that looked like in practice, on a React front-end I worked on.

## The Problem With Handing an Agent a Whole Repo

Modern coding agents are genuinely good, but only when the repository gives them something to hold on to — clear routing, stable examples, architectural boundaries, and some record of past decisions. Without that, an agent makes plausible but wrong assumptions. It adds a parallel routing model, invents backend persistence that was never there, duplicates a pattern that already exists, or drops a file in the wrong ownership boundary.
...
The idea is a lightweight AI governance and documentation system, and the front door to all of it is a single file: `AGENTS.md`.

It is the first thing an agent should read, and it deliberately does *not* try to contain every rule. Instead it routes — it points to the smallest relevant document for the task at hand. It tells the agent what the stack is, where the architecture docs live, which files own navigation, state, data, and styling, what kinds of changes require an ADR, and — just as importantly — what *not* to assume.

From there it branches into focused sections:

- **`docs/architecture/`** — how the system is actually built: repository map and ownership, runtime flow, the routing model, the data model, and a risk map of high-risk files and common agent mistakes.
- **`docs/standards/`** — the repeatable rules: components, styling, types, import boundaries, and when governance docs must be updated.
- **`docs/guides/`** — task-focused instructions like "how to add a new view" or "how to safely extend the data model."
...
**For humans,** it is the same win we always wanted and rarely wrote down: faster onboarding, cleaner reviews, less architectural drift, clearer ownership of state, data, styling, and navigation. New contributors read the same routing an agent does.
...
An AI-native repository is not a repository full of AI. It is a repository that is legible — one that can explain itself to whoever, or whatever, is about to change it. AGENTS.md is just the entry point. The real work is deciding, and writing down, how your system is meant to evolve.
